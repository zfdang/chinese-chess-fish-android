...
class XQFKey(object):
    def __str__(self):
        key_info = []
        key_info.append("XQF Keys:")
        key_info.append(f"  KeyXY (piece positions): {getattr(self, 'KeyXY', None)}")
        key_info.append(f"  KeyXYf (move start): {getattr(self, 'KeyXYf', None)}")
        key_info.append(f"  KeyXYt (move end): {getattr(self, 'KeyXYt', None)}")
        key_info.append(f"  KeyRMKSize: {getattr(self, 'KeyRMKSize', None)}")
        if hasattr(self, 'FKeyBytes'):
            key_info.append(f"  FKeyBytes: {[hex(b) for b in self.FKeyBytes]}")
        if hasattr(self, 'F32Keys'):
            key_info.append(f"  F32Keys: {[hex(b) for b in self.F32Keys]}")
        return "\n".join(key_info)
...
def read_from_xqf(full_file_name, read_annotation=True):
    ...
    if (version <= 0x0A):
        ...
        # dump keys here
        print(keys)  # This will print all the key information
